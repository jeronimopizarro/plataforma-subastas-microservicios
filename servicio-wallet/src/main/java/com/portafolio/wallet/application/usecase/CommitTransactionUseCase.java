...
@Service
public class CommitTransactionUseCase {

    private final WalletRepository walletRepository;
    private final TransactionRepository transactionRepository;

    public CommitTransactionUseCase(WalletRepository walletRepository, TransactionRepository transactionRepository) {
    ...
    public void execute(WalletTransactionCommand command) {
        ...
        wallet.commitFunds(command.amount());

        Transaction transaction = Transaction.create(wallet.getId(), command.amount(), TransactionType.COMMIT, command.reference());
