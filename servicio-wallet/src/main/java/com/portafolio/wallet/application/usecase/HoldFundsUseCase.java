...
@Service
public class HoldFundsUseCase {

    private final WalletRepository walletRepository;
    private final TransactionRepository transactionRepository;

    public HoldFundsUseCase(WalletRepository walletRepository, TransactionRepository transactionRepository) {
    ...
    public void execute(WalletTransactionCommand command) {
        ...
        wallet.holdFunds(command.amount());

        Transaction transaction = Transaction.create(wallet.getId(), command.amount(), TransactionType.HOLD, command.reference());
